...
class AnsibleSetupProvider(AbstractSetupProvider):
	...
	#define master_worker_ips
	worker_master_ips=defaultdict(list)
	for section,hosts in inventory_data.items():
		for host in hosts:
			worker_master_ips["{0}ips".format(section)].append("{0}".format(host[1]))
        with open(inventory_path, 'w+') as inventory_file:
            for section, hosts in inventory_data.items():
                ...
                if hosts:
                    ...
                    for host in hosts:
                        hostline = "{0} ansible_host={1} {2}".format(*host)
			for group,ips in worker_master_ips.items():
				hostline+=" {0}=".format(group)
				hostline+=str.join(",",ips)
			hostline+="\n"
