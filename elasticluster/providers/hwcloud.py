...
class OpenStackCloudProvider(AbstractCloudProvider):
    ...
    prePaid_id = None       
    prePaid_links = None        
    prePaid_security_groups = None

    def __init__(self, username, password, project_name, auth_url,
                 user_domain_name=None, project_domain_name="default",
    ...
    * run the instance with the given properties
    ...
    def create_prePaid_Server(self,node_name,image_id,flavorRef,**vm_start_args):
        pre_paid_args = {}
        pre_paid_args['name'] = node_name
        pre_paid_args['imageRef'] = image_id
        
        pre_paid_args['flavorRef'] = flavorRef   
        if 'key_name' in vm_start_args :        
            pre_paid_args['key_name'] = vm_start_args['key_name']
        if 'availability_zone' in vm_start_args :
            pre_paid_args['availability_zone'] = vm_start_args['availability_zone']
        if  'userdata' in vm_start_args :
            pre_paid_args['user_data'] = vm_start_args['userdata']
        if 'security_groups' in vm_start_args :         
            security_groups = vm_start_args['security_groups']
            security_groups_id = []
            for security_group_name in security_groups:             
                security_id = self.neutron_client.list_security_groups(retrieve_all=True,name=security_group_name)['security_groups'][0]['id']  
                security_groups_id.append({'id':security_id})        
            pre_paid_args['security_groups_id'] = security_groups_id
        
        if 'nics' in vm_start_args :
            nics = vm_start_args['nics']
            nics_prepaid = []       
            subnet_id = None    
            for nic in nics : 
                subnet_id = nic['net-id']                       
                ip_address = nic['v4-fixed-ip']                                         
                nics_prepaid.append({'subnet_id':subnet_id,'ip_address':ip_address})
            network_list = self.neutron_client.list_networks(retrieve_all=True)['networks']  
            for network in network_list:
                if subnet_id == network['id']:
                    vpc_id = network['name'] 
                    pre_paid_args['vpcid'] = vpc_id
            pre_paid_args['nics'] = nics_prepaid                        
            pre_paid_args['vpc_id'] = vpc_id
        chargingMode = "prePaid"
        periodType = vm_start_args.pop('period_type')
        periodNum = vm_start_args.pop('period_num')        
        if 'is_auto_renew' in vm_start_args :
            isAutoRenew = vm_start_args['is_auto_renew']
        else:
            isAutoRenew = "false"
        
        if 'is_auto_pay' in vm_start_args:
            isAutoPay = vm_start_args['is_auto_pay']
        else:
            isAutoPay = "false"
        
        pre_paid_args['extendparam'] = {
            "chargingMode": chargingMode,
            "periodType": periodType,
            "periodNum": periodNum,
            "isAutoRenew": isAutoRenew,
            "isAutoPay": isAutoPay,
            "regionID":self._os_region_name ,         
        }
        # get project-id,user_id,domain_id
        self.get_project_id()
    ...
    def get_project_id(self):
        ...
        auth_para = {"auth": {"identity": {"methods": ["password"],
                                "password": {"user": {"name":self._os_username,"password":self._os_password,
                                "domain": {"name":self._os_project_domain_name}}}},
                                "scope": {"project": {"name": self._os_tenant_name}}}} 
        
        auth_para=json.dumps(auth_para)    
        try:
            auth_response = requests.post('%s/auth/tokens'%self._os_auth_url, headers=auth_headers, data=auth_para)
        except requests.ConnectionError as msg:
            raise ResponseError(str(msg))
        ...
        self.token = auth_response.headers['X-Subject-Token']                
        self.project_id =  auth_response.json()['token']['project']['id'] 
        self.user_domain_id = auth_response.json()['token']['user']['domain']['id']
        self.user_id = auth_response.json()['token']['user']['id'] 
        pass    
    
             
    def query_prePaidRes(self,order_id):
        '''      
        query prePaidRes
...
class VmInfo:
    def __init__(self):
        VmInfo.id = OpenStackCloudProvider.prePaid_id        
        #links = OpenStackCloudProvider.prePiad_links
        #security_groups = OpenStackCloudProvider.prePaid_security_groups       
    

if __name__ == '__main__':
       #print OpenStackCloudProvider._decrypt("8785c8623ab262ab3032f088640d672c")
    cloud_provider = OpenStackCloudProvider(username= "cabc2558a81d512f49dac66bcab6e441",
                                            password="0f9bebc3e9544294357997267a9bd8b0",
                                        auth_url="https://iam.cn-north-1.myhwclouds.com/v3",project_name="cn-north-1",
                                    user_domain_name="yanjiao",project_domain_name="yanjiao",region_name="cn-north-1",
                                    storage_path="/root/.hwcc/storage",request_floating_ip=False)
    cloud_provider.get_project_id()
    #user_key_name = "hwckey"
    #user_key_public = "/root/.ssh/id_rsa.pub"
    #user_key_private = "/root/.ssh/id_rsa"
    #security_group = "hpc-hwc"
    #flavor = "s3.small.1"
    #image_id = "1189efbf-d48b-46ad-a823-94b942e2a000"
    #image_userdata = None 
    #availability_zone = "cn-north-1a",
    #username = "root"
    #extra={'login': 'linux', 'accelerator_count': 0, 'min_num': 1, 'network_ids': '328e440a-715c-48b9-a60a-f3b9d0fe1b85'}
    #nodename = "test-api-worker001 "   
    #instance_id = cloud_provider.start_instance(
        #user_key_name, user_key_public, user_key_private,
        #security_group,flavor, image_id, image_userdata,availability_zone=availability_zone,
        #username=username, node_name=nodename,
        #chargingMode = "prePaid",
        ##periodType = None,periodNum = None,
        ##isAutoRenew = None,isAutoPay = None,support_auto_recovery =None,
        #**extra)    
    #6ffb "instance_id: %s"%self.instance_id
